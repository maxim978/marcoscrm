...
import { Menu, X, Home, Disc, Users, Search, Video, LogOut, Globe, Music, Radio } from 'lucide-react'
      ...
      {isOpen && (
        <div className="absolute top-20 left-0 w-full bg-[#3071d8] border-t border-white/10 shadow-2xl z-50 animate-in slide-in-from-top duration-300">
          <nav className="flex flex-col p-6 gap-3">
            ...
            <Link href="/dashboard/contacts" onClick={toggle} className="flex items-center gap-4 rounded-xl px-4 py-4 text-white hover:bg-white/10 active:bg-white/20 transition-colors font-bold">
              <Users className="h-6 w-6 text-[#dfb433]" /> Algemene Contacten
            </Link>
            ...
            <Link href="/dashboard/radio" onClick={toggle} className="flex items-center gap-4 rounded-xl px-4 py-4 text-white hover:bg-white/10 active:bg-white/20 transition-colors font-bold">
              <Radio className="h-6 w-6 text-[#dfb433]" /> Radiostations
            </Link>
